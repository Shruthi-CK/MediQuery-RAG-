...
\begin{titlepage}
    ...
    {\large GENEARTIVE AI 285N $\cdot$ MSBA 285 \par}
...
primary forms: \textit{National Coverage Determinations} (NCDs), which establish
...
\begin{tabular}{@{}p{4.8cm}p{3.2cm}p{3.8cm}r@{}}
\toprule
\textbf{Document Type} & \textbf{Format} & \textbf{Source} & \textbf{Count} \\
\midrule
National Coverage Determinations  & CSV bulk export   & CMS Coverage Database & 343 \\
Local Coverage Determinations     & Excel bulk export & CMS Coverage Database & 819 \\
Medicare Benefit Policy Manual    & PDF (per chapter) & CMS IOMs              & 17 \\
Medicare Claims Processing Manual & PDF (per chapter) & CMS IOMs              & 39 \\
\midrule
...
All 1,162 processed documents were merged into a single \texttt{all\_chunks.json} file,
...
\midrule
\textbf{Total}                   & \textbf{1,162} & \textbf{3,377} & \textbf{2.91} \\
\bottomrule
\end{tabular}
\end{table}

The higher average chunk count for LCDs reflects their greater length: LCD documents
typically contain detailed coverage criteria, billing and diagnosis codes, and
documentation requirements, whereas NCDs are often more concise policy statements.
The complete corpus in \texttt{all\_chunks.json} serves as input to all downstream
pipeline components.
...
\begin{todobox}
\textbf{TO DO --- Person 4 (LLM Generation + Citations)}

\begin{itemize}[noitemsep, topsep=4pt]
    \item Describe the choice of Mistral-7B-Instruct as the generator
    \item Show the full prompt template used for grounded generation
    \item Explain the ``answer only from evidence'' and ``insufficient evidence''
          instructions and why they matter for hallucination reduction
    \item Show 2--3 example outputs: the question, retrieved chunks, and generated answer
          with citations
    \item Compare a baseline output (no RAG) vs.\ RAG output on the same question to
          illustrate hallucination reduction
    \item Describe any prompt engineering iterations that improved output quality
\end{itemize}
\end{todobox}
...
We constructed a test set of 25 questions spanning five coverage categories:
general medical procedures (5), durable medical equipment (5), home health services (5),
Texas-specific LCD queries (5), and mental health coverage (5). For each question, we
manually identified the ground-truth source document(s) from the corpus.

\subsection{Evaluation Metrics}

We evaluate three system configurations --- (1) Baseline LLM with no retrieval,
(2) RAG, and (3) RAG with query rewriting --- on the following metrics:

\begin{itemize}[noitemsep, topsep=4pt]
    \item \textbf{Recall@5}: Proportion of test questions for which the correct source
          document appears in the top 5 retrieved chunks.
    \item \textbf{Mean Reciprocal Rank (MRR)}: Average reciprocal rank of the first
          correct document across all queries.
    \item \textbf{Answer Correctness}: Binary human judgment of factual correctness.
    \item \textbf{Faithfulness}: Whether all claims are supported by retrieved chunks
          (no extrinsic hallucination).
    \item \textbf{Citation Accuracy}: Whether cited NCD/LCD IDs correctly correspond
          to the source of the answer.
    \item \textbf{Hallucination Rate}: Proportion of answers containing at least one
          unsupported factual claim.
\end{itemize}

\begin{todobox}
\textbf{TO DO --- Person 5 (Evaluation + UI)}

\begin{itemize}[noitemsep, topsep=4pt]
    \item Run all 25 test questions through all 3 system configurations
    \item Fill in the results table below with actual numbers
    \item Add qualitative analysis: which question types improved most? where did
          the system still fail?
    \item Describe the Streamlit UI and include a screenshot if possible
\end{itemize}
\end{todobox}
...
\toprule
\textbf{System} & \textbf{Recall@5} & \textbf{MRR} & \textbf{Correctness} & \textbf{Faithful.} & \textbf{Citation Acc.} & \textbf{Halluc. Rate} \\
\midrule
Baseline LLM        & ---      & ---      & [TO DO] & [TO DO] & [TO DO] & [TO DO] \\
RAG                 & [TO DO]  & [TO DO]  & [TO DO] & [TO DO] & [TO DO] & [TO DO] \\
RAG + Query Rewrite & [TO DO]  & [TO DO]  & [TO DO] & [TO DO] & [TO DO] & [TO DO] \\
\bottomrule
\end{tabular}
\end{table}
% ─────────────────────────────────────────────────────────────────────────────
...
Our corpus construction produced 3,377 well-tagged chunks from 1,162 policy documents,
